...
| G2M-R08 | Display creation and input/event binding use one shared Even bridge lifecycle without independent acquisition races. | Candidate: `src/even/evenBridge.ts` shared `getEvenBridge`/bounded `waitForEvenBridge`; `src/boundedRetry.test.mjs`; `src/main.ts` passes one bridge to display/input and uses one idempotent cleanup path for terminal lifecycle events and `pagehide`. | `GAP` | Lifecycle and transient-recovery behavior still require simulator/physical runtime evidence and independent review. |
| G2M-R09 | Switching between list and text container types uses page-container rebuilds; in-place text updates use upgrades. | Candidate: `src/even/evenDisplay.ts` `performRender` uses rebuilds across container kinds and text upgrades within kind; `src/serialQueue.ts` keeps later renders retryable after failure; `src/serialQueue.test.mjs`. | `GAP` | List-to-text, text-to-list, and failed-update recovery require simulator and physical G2 evidence. |
...
| G2M-R11 | Input binding and lifecycle failures are visible through sanitized phone-side diagnostics rather than silently swallowed. | Candidate: lifecycle classification tests in `src/even/evenInput.test.mjs`; `src/main.ts` `handleEvenLifecycle`/`cleanupEvenIntegration`/diagnostic updates; `src/renderer.ts` input and lifecycle rows; generic strings only. | `GAP` | Phone diagnostic lifecycle/failure rendering still requires runtime smoke and independent review. |
...
| G2M-R14 | Phase 1 build, available automated checks, simulator navigation, and physical G2 validation all pass. | Candidate: `npm install`, `npm run build`, `npm test` (navigation, SDK event routing/lifecycle, render queue recovery, bounded retry, and duplicate burst), and `git diff --check` passed. Simulator launch remains unavailable because required host libraries are absent; no physical G2 validation was performed. | `GAP` | Simulator navigation and every physical G2 checklist item remain open. |
