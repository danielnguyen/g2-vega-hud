...
|---|---|---|---|---|
| G2M-R01 | Application launch shows only Dashboard and Ask as sibling glasses-facing modes. | Candidate: `src/navigation.ts` `G2_HOME_ITEMS`/`initialG2Navigation`; `src/even/evenDisplay.ts` `startupHomePage`; `src/navigation.test.mjs`. | `GAP` | Implementation-pass evidence only; simulator and physical G2 launch still require independent review. |
| G2M-R02 | The top-level Dashboard/Ask menu uses an Even SDK native list container. | Candidate: `src/even/evenDisplay.ts` `homeList` creates `ListContainerProperty` with `isEventCapture: 1`. | `GAP` | Native rendering and selection still require simulator and physical G2 evidence. |
| G2M-R03 | Selecting Dashboard opens a text placeholder with no dashboard API calls, data retrieval, or gateway integration. | Candidate: `src/even/evenDisplay.ts` `childContent`/`childPage`; `src/navigation.test.mjs`; implementation diff contains no Dashboard request path. | `GAP` | Placeholder and negative-request behavior still require runtime observation and independent review. |
| G2M-R04 | Selecting Ask before bots are configured opens a concise setup state directing configuration to the phone surface. | Candidate: `src/even/evenDisplay.ts` `childContent`; `src/navigation.test.mjs`; implementation diff contains no Ask request or integration path. | `GAP` | Setup rendering and negative-request behavior still require runtime observation and independent review. |
| G2M-R05 | Single tap on a list selects and opens the highlighted item. | Candidate: `src/even/evenInput.ts` `normalizeEvenHubEvent` resolves native list selection with `currentSelectItemIndex ?? 0`; `src/main.ts` `handleInput`; index 1 and omitted-index cases in `src/even/evenInput.test.mjs`; navigation tests. | `GAP` | Simulator could not launch in this environment; physical G2 tap/selection evidence is required. |
| G2M-R06 | Double tap consistently climbs one navigation level from Dashboard and Ask child screens. | Candidate: `src/navigation.ts` `back-or-exit`; `src/main.ts` `handleInput`; `src/navigation.test.mjs` one-level back test. | `GAP` | Simulator and physical G2 double-tap evidence are required. |
| G2M-R07 | Double tap at the top-level menu invokes the normal host-managed exit flow. | Candidate: `src/even/evenInput.ts` system double-press mapping and focused test; `src/navigation.ts` `request-host-exit`; `src/even/evenDisplay.ts` `requestHostExit` calls `shutDownPageContainer(1)` without cleanup; navigation test. | `GAP` | Host confirmation/cancel/exit behavior requires physical G2 evidence. |
| G2M-R08 | Display creation and input/event binding use one shared Even bridge lifecycle without independent acquisition races. | Candidate: `src/even/evenBridge.ts` shared `getEvenBridge`/bounded `waitForEvenBridge`; `src/boundedRetry.test.mjs`; `src/main.ts` passes one bridge to display/input, uses one idempotent cleanup path, and invokes queued display `refresh` on foreground entry; focused display refresh tests. | `GAP` | Lifecycle resume, cleanup, and transient-recovery behavior still require simulator/physical runtime evidence and independent review. |
| G2M-R09 | Switching between list and text container types uses page-container rebuilds; in-place text updates use upgrades. | Candidate: `src/even/evenDisplay.ts` normal `performRender` preserves rebuild/upgrade behavior while `performRefresh` forces the current list/text rebuild; `src/even/evenDisplay.test.mjs` covers home, child, failure, and later recovery; recoverable queue tests. | `GAP` | List-to-text, text-to-list, foreground rebuild, and failed-update recovery require simulator and physical G2 evidence. |
| G2M-R10 | Duplicate firmware events are debounced without suppressing intentional navigation. | Candidate: `src/inputDedupe.ts` advances the exact-key 120 ms window across a duplicate burst; `src/main.ts` `handleEvenInput`; `src/inputDedupe.test.mjs` three-event burst, quiet-period, and distinct rapid-action cases. | `GAP` | Firmware timing and rapid intentional navigation require physical G2 evidence. |
| G2M-R11 | Input binding and lifecycle failures are visible through sanitized phone-side diagnostics rather than silently swallowed. | Candidate: lifecycle classification tests; `src/main.ts` `handleEvenLifecycle` reports forced foreground-refresh rejection through `recordEvenFailure`; cleanup/diagnostic updates and phone lifecycle rows use generic strings only; failed refresh is observable in focused tests. | `GAP` | Phone diagnostic lifecycle/refresh-failure rendering still requires runtime smoke and independent review. |
| G2M-R12 | Existing phone-side configuration, runtime status, and debug surfaces continue to render during Phase 1. | Candidate: `src/renderer.ts` retains settings/status/debug; `src/main.ts` retains save/clear/test-connection paths; `npm run build` passed. | `GAP` | Simulator phone smoke was unavailable and physical/host phone rendering still requires validation. |
| G2M-R13 | Phase 1 introduces no Telegram, GramJS, speech-to-text, microphone, bot-configuration, or new credential behavior. | Candidate: implementation package diff adds only the focused-test command; lockfile/manifest implementation diff is empty; source diff is limited to navigation, display/input lifecycle, placeholders, and generic diagnostics. | `GAP` | Independent complete-diff and packaged-artifact privacy/exclusion review remains required. |
| G2M-R14 | Phase 1 build, available automated checks, simulator navigation, and physical G2 validation all pass. | Candidate: `npm install`, `npm run build`, `npm test` (including forced home/child refresh and post-failure recovery), and `git diff --check` passed. Simulator launch remains unavailable because required host libraries are absent; no physical G2 validation was performed. | `GAP` | Simulator navigation, lifecycle refresh, and every physical G2 checklist item remain open. |
...
| G2M-R48 | Repeated listen/finalize/cancel cycles work on physical G2 hardware without stale sessions or duplicate transitions. | Phase 4 exit gate in `plan/README.md`. No physical evidence. | `GAP` | Run repeated device validation. |
...
| G2M-R68 | Repeated question/response cycles work with multiple configured bots without cross-chat leakage. | Phase 6 exit gate in `plan/README.md`. No runtime evidence. | `GAP` | Run multi-bot device and Telegram validation using non-committed configuration. |
...
| G2M-R81 | Production builds, automated tests, formatting/diff checks, and package validation pass. | Phase 7 exit gate in `plan/README.md`. No final evidence. | `GAP` | Record final commands and results. |
...
| G2M-R83 | Deprecated PR #8 remains unmerged and its relevant shared-bridge/input behavior is either demonstrably superseded or explicitly recorded as residual reference-only work. | `plan/README.md` entry gate; PR #8 title marks it deprecated/do-not-merge. No supersession review yet. | `GAP` | Complete after Phase 1 and confirm again at final conformance. |
...
| G2M-R85 | A final specification-based review audits every mandatory checklist row against merged implementation and evidence. | Phase 7 conformance gate in `plan/README.md`. No final audit. | `GAP` | Run independent conformance review after implementation PRs merge. |
| G2M-R86 | Every mandatory row is `PASS` or approved `AMENDED`, with no unresolved `GAP`. | Closure rule in `plan/README.md`. Current checklist intentionally begins with unresolved gaps. | `GAP` | Close only after later review proves all rows. |
