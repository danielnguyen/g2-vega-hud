...
| G2M-R05 | Single tap on a list selects and opens the highlighted item. | Candidate: `src/even/evenInput.ts` `normalizeEvenHubEvent`; `src/main.ts` `handleInput`; `src/navigation.ts` `select`/`open-selected`; focused navigation tests. | `GAP` | Simulator could not launch in this environment; physical G2 tap/selection evidence is required. |
...
| G2M-R07 | Double tap at the top-level menu invokes the normal host-managed exit flow. | Candidate: `src/navigation.ts` `request-host-exit` effect; `src/even/evenDisplay.ts` `requestHostExit` calls `shutDownPageContainer(1)`; focused navigation test. | `GAP` | Host confirmation/exit behavior requires physical G2 evidence. |
| G2M-R08 | Display creation and input/event binding use one shared Even bridge lifecycle without independent acquisition races. | Candidate: `src/even/evenBridge.ts` `getEvenBridge`; `src/main.ts` `initializeEvenIntegration` passes one bridge to display and input and releases the handler on `pagehide`. | `GAP` | Lifecycle behavior still requires simulator/physical runtime evidence and independent review. |
| G2M-R09 | Switching between list and text container types uses page-container rebuilds; in-place text updates use upgrades. | Candidate: `src/even/evenDisplay.ts` `performRender` uses `rebuildPageContainer` across container kinds and `textContainerUpgrade` within text kind. | `GAP` | List-to-text and text-to-list rendering require simulator and physical G2 evidence. |
| G2M-R10 | Duplicate firmware events are debounced without suppressing intentional navigation. | Candidate: `src/inputDedupe.ts` exact-key 120 ms bound; `src/main.ts` `handleEvenInput`; `src/inputDedupe.test.mjs` duplicate, distinct rapid-action, and repeated-input cases. | `GAP` | Firmware timing and rapid intentional navigation require physical G2 evidence. |
| G2M-R11 | Input binding and lifecycle failures are visible through sanitized phone-side diagnostics rather than silently swallowed. | Candidate: `src/main.ts` `updateEvenInputBinding`/`recordEvenFailure`; `src/renderer.ts` `formatInputBinding`; generic diagnostic strings only. | `GAP` | Phone diagnostic failure rendering still requires runtime smoke and independent review. |
...
| G2M-R14 | Phase 1 build, available automated checks, simulator navigation, and physical G2 validation all pass. | Candidate: `npm install`, `npm run build`, `npm test`, and `git diff --check` passed. Simulator launch failed because required WebKitGTK/JavaScriptCoreGTK 4.1 host libraries were unavailable; no physical G2 validation was performed. | `GAP` | Simulator navigation and every physical G2 checklist item remain open. |
